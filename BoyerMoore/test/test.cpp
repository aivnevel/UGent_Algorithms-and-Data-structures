...
struct TekstZoekTest
{
	...
	int aantalVergelijkingenHeuristiekVerkeerdeKarakterAlternative;
	int aantalVergelijkingenHorspool;
	int aantalVergelijkingenSunday;
};

const TekstZoekTest tztests[] = {
	{"abra", "abracadabra", 2, 10, 10, 9, 9},
	{"a", "abcabcabcabcdaabcdef", 6, 20, 20, 20, 12},
	{"aabaac", "aabaacaacaabaabaac", 2, 22, 20, 14, 20},
	{"aabaac", "aabaacdacaabaabaac", 2, 16, 16, 14, 15},
	{"aaa", "aaaaaaaaaaaaaaaaaaaa", 18, 54, 54, 54, 54}, //ouch!
	{"anaxanax", "anaxananaxanax", 1, 16, 15, 14, 15},
	{"aaaaaaab", "aaaaaaxaaaaaay", 0, 7, 7, 7, 4},
};
...
TEST_CASE("BM met heuristiek van verkeerde karakter [alternative]", "[simpel]")
{

	for (auto tzt : tztests)
	{
		BMVerkeerdeKarakterAlternative bmvk(tzt.naald);
		
		auto plaatsen = bmvk.zoek(tzt.hooiberg);

		CHECK(plaatsen.size() == tzt.aantal);
		CHECK(bmvk.laatsteAantalKaraktervergelijkingen == tzt.aantalVergelijkingenHeuristiekVerkeerdeKarakterAlternative);
	}
}
