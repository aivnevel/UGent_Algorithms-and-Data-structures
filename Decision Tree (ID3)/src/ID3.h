...
bool ID3Tree::build(std::vector<std::string> &questions, std::list<std::string> &animals, std::vector<std::map<std::string, bool>> answers)
{
    std::vector<bool> usedQuestions(questions.size(), false);
    return build(questions, animals, answers, usedQuestions);
}

bool ID3Tree::build(std::vector<std::string> &questions, std::list<std::string> &animals, std::vector<std::map<std::string, bool>> &answers, std::vector<bool> usedQuestions)
{
    if (animals.size() == 1)
    {
    ...
    // Look for the question that best splits the remaining animals
    ...
    if (selectedQuestion > -1)
    {
        ...
        *this = std::make_unique<ID3Node>();
        ...
        if (animalsLeft.size() == 0 || animalsRight.size() == 0)
        {
            ...
            return build(questions, (animalsLeft.size() == 0 ? animalsRight : animalsLeft), answers, usedQuestions);
        }
        else
        {
            ...
            {
                result = (*this)->left.build(questions, animalsLeft, answers, usedQuestions);
            }
            if (animalsRight.size() > 0)
            {
                result &= (*this)->right.build(questions, animalsRight, answers, usedQuestions);
            }
