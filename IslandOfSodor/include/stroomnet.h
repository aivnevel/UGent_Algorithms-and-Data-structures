...
//Copyconstructor. Let op: Graaf<GERICHT>(gr) moet toegevoegd worden,
...
//                    Graaf<GERICHT>(gr),GraafMetTakdata<GERICHT, T>(gr),van(0),naar(1){};
    ...
    Stroomnetwerk<T>& buildFlowNetwork(Pad<T> pad, bool isPlus);
...
template<class T>
Stroomnetwerk<T>& Stroomnetwerk<T>::buildFlowNetwork(Pad<T> pad, bool isPlus) {
    for (int i = 1; i < pad.size(); i++) {
        int capacity = pad.geefCapaciteit();
        ...
        T *currConnection = this->geefTakdata(from, to);
        
        if (currConnection != 0) {
            *currConnection += capacity;
        } else {
            this->voegVerbindingToe(from, to, capacity);
        }
        
        T *restConnection = this->geefTakdata(to, from);
        if (restConnection != 0) {
            *restConnection -= capacity;
            if (*restConnection == 0) {
                this->verwijderVerbinding(to, from);
            }
        }
    }
...
template<class T>
Stroomnetwerk<T>& Stroomnetwerk<T>::operator+=(Pad<T> pad) {
    return buildFlowNetwork(pad, true);
}

template<class T>
Stroomnetwerk<T>& Stroomnetwerk<T>::operator-=(Pad<T> pad) {
    return buildFlowNetwork(pad, false);
}
