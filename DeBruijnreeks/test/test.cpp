...
{
	// if the sequence is not og length M^N (M: alfabet_lengte, N: orde), then it is not a valid sequence
	...
	// create a copy of the string and add the first (orde-1) chars to the end of it
	...
	for (int i = 0; i < dbr.length(); i++) {
		// the sequence is invalid if a letter is not in the given alphabet
		...
		// add the current permutation to the history
