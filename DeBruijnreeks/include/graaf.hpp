...
public:
    ...
    Takdata *geefTakdata(int verbindingsnummer);
...
template <RichtType RT, class Takdata>
Takdata *GraafMetTakdata<RT, Takdata>::geefTakdata(int verbindingsnummer) {
    if (takdatavector.size() <= verbindingsnummer)
        return nullptr;
    else if (verbindingsnummer != -1)
        return &takdatavector[verbindingsnummer];
    else
        return 0;
}
